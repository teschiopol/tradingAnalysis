...
def ricerca():
    ...
    if list_report == [] or list_report[0] == '':
        ...
        rep = conn.execute(
            select([reports.c.name, reports.c.id], reports.c.id.in_(list_report)).order_by(reports.c.name))
    rep_iter = list(rep)
    rep_iter_single = []
    for d in rep_iter:
    ...
    if data_al == '':
        ...
        data_final = conn.execute(
            select([func.max(dati.c.data).label('maxdata')], dati.c.idreport.in_(rep_iter_single)))
    ...
    if data_dal == '':
        ...
        data_final = conn.execute(
            select([func.min(dati.c.data).label('mindata')], dati.c.idreport.in_(rep_iter_single)))
    ...
    app.jinja_env.globals['today'] = datetime.datetime(int(data_final[:4]), int(data_final[5:7]),
                                                       int(data_final[8:10])).strftime("%Y-%m-%d")

    res = conn.execute(select(
        [func.sum(dati.c.daily_profit_loss), dati.c.data, func.max(dati.c.somma_trade), dati.c.anno, dati.c.mese],
        dati.c.idreport.in_(rep_iter_single)).where(and_(dati.c.data >= data_dal, dati.c.data <= data_al)).group_by(
        dati.c.data, dati.c.anno, dati.c.mese).order_by(dati.c.data))
    pivot_data = list(res)
    unisci_data = pivot_data
    report = conn.execute('SELECT * FROM reports order by name')
    mes_ann = conn.execute(select(
        [func.sum(dati.c.daily_profit_loss), dati.c.anno, dati.c.mese, func.min(dati.c.daily_profit_loss),
         func.max(dati.c.daily_profit_loss), func.avg(dati.c.daily_profit_loss)],
        dati.c.idreport.in_(rep_iter_single)).where(and_(dati.c.data >= data_dal, dati.c.data <= data_al)).group_by(
        dati.c.anno, dati.c.mese).order_by(dati.c.anno, dati.c.mese))
    conn.close()
...
def graph():
    ...
    mesi_nome = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre',
                 'Ottobre', 'Novembre', 'Dicembre']
    master_ret = [[]]
    if strategie[0] == '0':
        res = conn.execute(select([func.sum(dati.c.daily_profit_loss), dati.c.anno, dati.c.mese]).where(
            and_(dati.c.data >= dataI, dati.c.data <= dataF)).group_by(dati.c.anno, dati.c.mese).order_by(dati.c.anno,
                                                                                                          dati.c.mese))
        ...
        for x in strategie:
            sup_query = conn.execute(select([func.sum(dati.c.daily_profit_loss), dati.c.anno, dati.c.mese]).where(
                and_(dati.c.data >= dataI, dati.c.idreport == x, dati.c.data <= dataF)).group_by(dati.c.anno,
                                                                                                 dati.c.mese).order_by(
                dati.c.anno, dati.c.mese))
        ...
        nome_sup_query = conn.execute(select([reports.c.id, reports.c.name], reports.c.id.in_(strategie)))
        nome_sup_query = list(nome_sup_query)
        res = conn.execute(select([func.sum(dati.c.daily_profit_loss), dati.c.anno, dati.c.mese],
                                  dati.c.idreport.in_(strategie)).where(
            and_(dati.c.data >= dataI, dati.c.data <= dataF)).group_by(dati.c.anno, dati.c.mese).order_by(dati.c.anno,
                                                                                                          dati.c.mese))
